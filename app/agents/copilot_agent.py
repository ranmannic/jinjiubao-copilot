...
from app.core.customer_channels import infer_channels_from_type
...
from app.core.need_steps import RETAIL_BANDS, apply_retail_band, current_need_step
...
class CopilotAgent:
    ...
    async def start_session(
        ...
        is_new_id = self.registry.is_new(customer_id)
        is_returning = not is_new_id
        ...
        if customer.get("region"):
            ...
            if len(parts) >= 2:
                ...
                if not profile.needs.region_city:
                    profile.needs.region_city = parts[1]

        from app.services.session_service import new_session_id

        if is_returning:
            start_phase = ConversationPhase.NEED_DISCOVERY
        else:
            start_phase = ConversationPhase.TYPE_IDENTIFICATION
        ...
        session.metadata["is_new_customer"] = is_new_id
        history_list = self.history.list_sessions(customer_id)

        llm_turn, llm_err = await self.llm.generate_welcome(session)
        if llm_turn and llm_turn.reply and not is_returning:
            msg = llm_turn.reply
            if is_new_id and "初次见面" not in msg:
                msg = f"初次见面，{msg}"
            replies = default_quick_replies(session)
        else:
            msg, replies = welcome_message(profile, is_new=is_new_id)
            if llm_err:
                msg = f"（AI 对话暂不可用：{llm_err}）\n\n{msg}"

        session.messages.append(ChatMessage(role="assistant", content=msg))
        self.registry.mark_seen(customer_id)
        resp = self._response(session, msg, replies, history_sessions=history_list)
    ...
    async def _handle_rule_based(
        ...
        if text == "custom_need":
            msg = "请直接输入您的需求：想找什么品类、口感、期望零售价位等。"
            session.messages.append(ChatMessage(role="assistant", content=msg))
            return self._response(session, msg, default_quick_replies(session)), session
        ...
        if phase == ConversationPhase.TYPE_IDENTIFICATION:
            if profile.is_returning:
                ...
                prompt, replies = get_quick_replies_for_session(session)
                msg = prompt or "请描述您这次想找什么产品："
            elif profile.customer_type != CustomerType.UNKNOWN:
                infer_channels_from_type(profile)
                session.phase = ConversationPhase.INQUIRY
                msg = inquiry_city_message()
                replies = default_quick_replies(session)
            else:
                msg, replies = welcome_message(profile, is_new=session.metadata.get("is_new_customer", True))
            ...
            if not profile.needs.region_city:
                ...
                from app.core.quick_replies import get_quick_replies_for_session
                prompt, replies = get_quick_replies_for_session(session)
                msg = prompt or "您这次主要想找哪类酒？"
        elif phase == ConversationPhase.NEED_DISCOVERY:
            if needs_complete(session.profile.needs) or current_need_step(profile, session.metadata) == "done":
                return await self._recommend(session)
            from app.core.quick_replies import get_quick_replies_for_session
            prompt, replies = get_quick_replies_for_session(session)
            msg = prompt or "请继续补充选品需求。"
    ...
    async def _update_profile_from_input(
        ...
        if qr.startswith("need_cat:"):
            cat = qr.split(":", 1)[1]
            if cat not in profile.needs.categories:
                profile.needs.categories.append(cat)
            return

        if qr.startswith("need_taste:"):
            taste = qr.split(":", 1)[1]
            if taste == "skip":
                session.metadata["need_step"] = "taste_done"
            elif taste not in profile.needs.taste_preferences:
                profile.needs.taste_preferences.append(taste)
            return

        if qr.startswith("retail_"):
            apply_retail_band(profile.needs, qr)
            band = RETAIL_BANDS.get(qr)
            if band:
                _, hi, _ = band
                profile.needs.supply_price_max = (hi if hi < 99999 else 2000) * 0.55
            return
    ...
    async def _recommend(
        ...
        eligible, blocked = recommend_products(products, profile, competition, limit=3)
        session.recommendations = eligible
        session.blocked_recommendations = blocked
        ...
        if intro:
            rec_part, replies = recommendation_message(profile, eligible)
            msg = f"{intro}\n\n{rec_part}"
        else:
            msg, replies = recommendation_message(profile, eligible)
        ...
        session.messages.append(ChatMessage(role="assistant", content=msg))
        return self._response(session, msg, replies, recommendations=eligible), session
    ...
    async def _do_handoff(
        ...
        lead = build_crm_lead(
            ...
            session.session_id,
            session.recommendations,
            session.blocked_recommendations,
        )
        ...
        return CopilotResponse(
            ...
            blocked_recommendations=session.blocked_recommendations,
    ...
    def _persist_history(self, session: ConversationSession) -> None:
        from datetime import datetime

        needs = session.profile.needs
        summary_bits = [
            *(needs.categories or ["未选品类"]),
        ]
        if needs.region_city:
            summary_bits.append(needs.region_city)
        ...
        self.history.save_session(
            session.customer_id,
            {
                ...
                "messages": [{"role": m.role, "content": m.content} for m in session.messages[-40:]],
                "profile": session.profile.model_dump(mode="json"),
                "recommendations": [r.model_dump(mode="json") for r in session.recommendations],
                "blocked_recommendations": [
                    r.model_dump(mode="json") for r in session.blocked_recommendations
                ],
            },
        )
    ...
    def _response(
        ...
        return CopilotResponse(
            ...
            metadata=meta,
            history_sessions=history_sessions or self.history.list_sessions(session.customer_id),
        )
